...
function renderProducts(allProducts) {
  ...
  for (let product of allProducts) {
      ...
      let discountAmount = product.discountPercentage ? (originalPrice * product.discountPercentage / 100).toFixed(0) : 0;
      ...
      <!-- İndirim oranı badge'ı -->
      ${product.discountPercentage ? `<div class="discount-badge ">% ${product.discountPercentage.toFixed()}</div>` : ''}
          ...
          <div class="action-buttons mt-4">
              <button onclick="openEditModal(${product.id})" class="btn border border-warning btn-edit btn-sm">Düzenle</button>
              <button onclick="deleteP(${product.id})" class="btn btn-danger btn-sm">Sil</button>
          </div>
      </div>`;
  }
  document.getElementById('productsList').innerHTML = output;
}
...
function openEditModal(id) {
  ...
  if (product) {
    ...
    const bsModal = new bootstrap.Modal(editProductModal)
    bsModal.show()
  }
}
...
console.log(title);
 ...
 allProducts = editProduct({ id, title, description })
...
const filteredProductList = filterProducts(query);
...
document.querySelectorAll('.modal-footer button').forEach(e=>e.addEventListener('click', closeModal) ) 
...
const newProductsList = createNewProduct({ title, description, image });
renderProducts(newProductsList);
